...
export default function Comets() {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return; // Ensure the component is client-side
    ...
    const comets: Comet[] = Array.from({ length: 8 }, (_, index) => ({
      ...
      active: false,
      delay: index * 100, // 3-second delay between each comet
    }));
    ...
    function drawComet(comet: Comet) {
      ...
      const tailLength = 130; // Longer tail
